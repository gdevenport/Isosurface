...
"""
function iso_vtk(name::String, iteration::Int, U::Bool, W::Bool)
...
function part_folder_isosurface()
    ...
    for i in file_start:file_end
        iso_vtk("$file_name.$i.h5",i,true, false)
        println("Creating Isosurface for file $i")
    end
end

part_folder_isosurface()